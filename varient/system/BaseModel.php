...
abstract class BaseModel
{
    ...
    public function paginate(?int $perPage = null, string $group = 'default', ?int $page = null, int $segment = 0)
    {
        ...
        // Get the total count first without resetting the query builder
        $totalResults = $this->countAllResults(false);

        // THE ARCHITECT FIX: If there are no results, do not execute the SELECT query!
        if ($totalResults === 0) {
            $this->pager = $pager->store($group, $page, $perPage, 0, $segment);

            // CRITICAL: We must reset the query builder manually because
            // countAllResults(false) kept the WHERE clauses in memory.
            $this->builder()->resetQuery();

            // Return empty array instantly, saving a useless database trip
            return [];
        }

        // Normal execution if records exist
        $this->pager = $pager->store($group, $page, $perPage, $totalResults, $segment);
